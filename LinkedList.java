...
public class LinkedList<E extends Comparable<E>> {
	...
	private void removeAll(E el, Node currNode) {
		...
		if (first.data.equals(el)) {
			...
			if (currNode.next.data.equals(el)) {
				currNode.next = currNode.next.next;
				n--;
				removeAll(el, currNode);
			} else {
				removeAll(el, currNode.next);
			}
		}
	}
	...
	private void duplicateAll(E el, LinkedList<E>.Node currNode) {
		if (currNode != null) {
			if (currNode.data.equals(el)) {
				currNode.next = new Node(el, currNode.next);
				n++;
				duplicateAll(el, currNode.next.next);
			} else {
				duplicateAll(el, currNode.next);
			}
		}
	}
